...
function CourierController.controlCourier()
	...
	for i = 1,#CourierController.courierArray do
		...
		if courier then
			...
			-- if was transferring to me, but now it isnt, tranfer to me
			if CourierController.forceTranfer[courier] == true and courierEnt and courierEnt ~= myHero and Entity.IsAlive(myHero) then
				Ability.CastNoTarget(transfer)
				Log.Write("some one trying to use courier")
			
			-- if was tranfering and it is done delivering .remove flag
			elseif CourierController.forceTranfer[courier] == true and NPC.IsEntityInRange(myHero, courier, 400) and Entity.IsAlive(myHero)then
				CourierController.forceTranfer[courier] = false

			-- if not using, but it is forbiden user, go back to base
			elseif courierEnt and  Entity.IsHero(courierEnt) and courierEnt~=myHero and CourierController.mutedHeroes[NPC.GetUnitName(courierEnt)] and not CourierController.forceTranfer[courier] then
...
function CourierController.getPlayerHeroMap()
	...
	local myHero = Heroes.GetLocal()
	--Log.Write("time:"..GameRules.GetGameTime().. " start:"..GameRules.GetGameStartTime())

	if not CourierController.players[0] then
		Log.Write("added heroes")
